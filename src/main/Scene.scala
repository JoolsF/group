...
object Scene {
  ...
  val ACTOR_SYSTEM = ActorSystem("MySystem")
  val COORDINATOR_ACTOR = ACTOR_SYSTEM.actorOf(Props[Coordinator], name = "ActorCoordRay")
...
class Scene private(val objects: List[Shape], val lights: List[Light]) {
  ...
  def traceImage(width: Int, height: Int) {
    ...
    for (y <- 0 until height) {
      val actorRef = system.actorOf(Props[SceneActor], name = "ActorRay" + y)
      actorRef !(height, width, y, objects, lights, system)
    }
  }
}

//SceneActor class is run concurrently
class SceneActor extends Actor {

  val system = Scene.ACTOR_SYSTEM
  val coord = Scene.COORDINATOR_ACTOR
